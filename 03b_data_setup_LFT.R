...
## Description: 03b_data_setup_LFT - Repeats linkage of testing and covariate data included in '03_data_setup'
## but incorporates lateral flow test results (and censors accordingly), whereas 03_data_setup identifies positive
## cases using PCR results only. The linked dataset produced in 03_data_setup is used in the matched analysis to  
## identify indicators of long COVID, whereas the dataset produced by this script is only used for classification of 
## individuals as having long COVID or not according to our operational definition i.e. classification of individuals
## as having long COVID according to our operational definition relaxes the requirement that individulas have a positive
## PCR test to be identified as positive cases, and instead, identifies individuals with positive PCR OR LFT tests as 
## positive cases. This script also diverges from 03_data_setup by omitting the creation of positive controls, which 
## is not required since the dataset produced by this script is not used for matching.
## Exports prepared data to: "/conf/EAVE/GPanalysis/analyses/long_covid/data/LongCOVID_linked_LFT.rds"
##############################################################################################################
...
df <- GP_raw %>% 
  ...
  mutate(age = as.numeric(chi_age),
         ...
         age = if_else(age >= 95 & age <= 99, 97, age),
         age = if_else(age >= 90 & age <= 94, 92, age),
         age = if_else(age >= 80 & age <= 89, trunc(age/2)*2, age), 
         age = as.numeric(age),
...
## Check whether to match on new or old EAVE_LINKNOs
table(df$EAVE_LINKNO %in% EAVE$EAVE_LINKNO) # 5,133,038
table(df$EAVE_LINKNO %in% EAVE$EAVE_LINKNO_old) # 5,148,268 --> suggests EAVE_LINKNO_old should be used
...
df <- df %>% 
  filter(EAVE_LINKNO %in% EAVE$EAVE_LINKNO_old) # 5,148,268
...
df <- df %>% 
  ...
  select(-EAVE_LINKNO_new) %>%  # 5,148,268
  # Remove individuals who have an old EAVE_LINKNO but no new EAVE_LINKNO
  filter(!is.na(EAVE_LINKNO)) # 5,139,041

rm(EAVE_raw, EAVE)


# 1.0b Remove likley ghost records ----

## Read in EAVE_LINKNOs to be removed
## File produced using code in 00_Cohort_checking
## Identifies individuals who are in the GP data but not in the validated cohort file (demographic file), and who are:
reweight <- readRDS("/conf/EAVE/GPanalysis/analyses/long_covid/outputs/1. Misc/EAVE_LINKNOs_to_reweight.rds") %>% 
  filter(EAVE_LINKNO %in% df$old_EAVE_LINKNO) %>% 
  rename("reweight" = eave_weights) %>% 
  filter(reweight == "Remove") # 427,806

## Remove individuals 60+ with no test date and no interaction with health system in recent years
df <- df %>% 
  filter(!(old_EAVE_LINKNO %in% reweight$EAVE_LINKNO)) # 4,711,235
...
## Remove rows with missing EAVE_LINKNOs
df <- df[!is.na(df$EAVE_LINKNO),]


# 1.2 Add positive lateral flow tests - use the earliest of PCR or lateral flow test to identify positive cases ----
## Read in data
lft_raw <- readRDS("/conf/EAVE/GPanalysis/data/lft_positives.rds")

## Get individuals' first positive LFT result
lft <- lft_raw %>% 
  rename("lft_tes_dat" = date_ecoss_specimen) %>% 
  select(-c(test_result, test_type)) %>% 
  group_by(EAVE_LINKNO) %>% 
  arrange(lft_tes_dat) %>% 
  slice(1)

## Check how LFT tests are distributed across existing test results
table(df$tes_res[df$EAVE_LINKNO %in% lft$EAVE_LINKNO]) 

## Check split of test results before incorporating lft results
prop.table(table(df$tes_res)) 

## Update test results and dates in df
df <- df %>% 
  ## If an individual has a positive PCR test and a positive LFT, use the earliest tes_dat
  left_join(lft, by = "EAVE_LINKNO") %>% 
  mutate(tes_dat = ifelse(!is.na(lft_tes_dat) & tes_res == "POSITIVE" & (lft_tes_dat < tes_dat), lft_tes_dat, tes_dat),
         tes_dat = as.Date(tes_dat, origin = "1970-01-01")) %>% 
  ## If an individual does not have a positive PCR test but has a positive LFT, update tes_res to positive and use the lft tes_dat
  mutate(tes_dat = ifelse(tes_res != "POSITIVE" & !is.na(lft_tes_dat), lft_tes_dat, tes_dat),
         tes_dat = as.Date(tes_dat, origin = "1970-01-01"),
         tes_res = ifelse(tes_res != "POSITIVE" & !is.na(lft_tes_dat), "POSITIVE", tes_res)) %>% 
  select(-lft_tes_dat)

## Check split of test results after incorporating lft results
prop.table(table(df$tes_res)) 

## Remove files
rm(lft) ## keep lft_raw for censoring
...
Q <- Q_raw %>%
  select(c(EAVE_LINKNO, Q_DIAG_AF:Q_DIAG_VTE, Q_DIAG_CKD_LEVEL, n_risk_gps),
         ...
         bmi = Q_BMI)  %>% 
  select(-c(Q_DIAG_DIABETES_1, Q_DIAG_DIABETES_2))


## Read in old file to get diabetes data and join to Q 
Q_old <- readRDS("/conf/EAVE/GPanalysis/progs/CR/Vaccine/output/temp/Qcovid.rds") %>% 
  select(EAVE_LINKNO, Q_DIAG_DIABETES_1, Q_DIAG_DIABETES_2)

Q <- Q_old %>% 
  left_join(Q, by = "EAVE_LINKNO") %>% 
  # Remove n_risk_gps -- use individual groups instead
  select(-n_risk_gps)
...
df <- df %>% 
  ...
  mutate_at(vars(Q_DIAG_DIABETES_1:Q_DIAG_DIABETES_2), ~replace_na(.,0)) %>% 
  mutate(Q_DIAG_CKD_LEVEL = ifelse(Q_DIAG_CKD_LEVEL > 0, 1, 0)) # binarise

## Recalculate n_risk_gps
Q_cols <- colnames(df)
Q_cols <- Q_cols[startsWith(Q_cols, "Q_")]

df$n_risk_gps <- df[, Q_cols] %>% 
  rowSums(na.rm = T)


## Read in imputed BMI (imputed using multiple imputation chained equations (MICE)) 
bmi_raw <- readRDS("/conf/EAVE/GPanalysis/analyses/imputation/data/df_imp.rds") %>% 
  distinct(EAVE_LINKNO, .keep_all = T)

df <- df %>% 
  left_join(dplyr::select(bmi_raw, EAVE_LINKNO, bmi_imp = Q_BMI), by = "EAVE_LINKNO") 
...
# Export an uncensored version of the df
saveRDS(df, "/conf/EAVE/GPanalysis/analyses/long_covid/data/LongCOVID_linked_LFT_uncensored.rds", compress = TRUE)


# NB All 'never tested' are censored for death or incomplete follow-up during matching, based on their positive match's test date
...
## Get dates of subsequent LFT or PCR positive tests > 42 days after initial positive test
## Merge PCR and LFT positive tests

lft <- lft_raw %>% 
  select(EAVE_LINKNO, date_ecoss_specimen) %>% 
  mutate(date_ecoss_specimen = date(date_ecoss_specimen))

all_pos_tests <- testing_raw %>% 
  filter(test_result == "POSITIVE") %>%
  select(EAVE_LINKNO, date_ecoss_specimen) %>% 
  mutate(date_ecoss_specimen = date(date_ecoss_specimen))%>% 
  bind_rows(lft)
  
subsequent_pos <- all_pos_tests %>% 
...
## Export
saveRDS(df, "/conf/EAVE/GPanalysis/analyses/long_covid/data/LongCOVID_linked_LFT.rds", compress = TRUE)
