...
table(df$EAVE_LINKNO %in% EAVE$EAVE_LINKNO) # 5,139,038
table(df$EAVE_LINKNO %in% EAVE$EAVE_LINKNO_old) # 5,148,268 --> suggests use EAVE_LINKNO_old
...
GP_max_date <- max(df$tes_dat, na.rm=T) 
...
## Testing data and study dates are missing for those who never tested (as expected)
## Positive test dates are missing for those who never tested positive (as expected)
## Missing values for death date, and BMI before imputation (as expected)
...
## Export
saveRDS(df, "/conf/EAVE/GPanalysis/analyses/long_covid/data/LongCOVID_linked.rds", compress = TRUE) # 4,635,186 unique EAVE_LINKNO
